...
load('MigVSWintIsrael.mat')
DateStart=datenum({'14/10/17'},'dd/mm/yy');
DateEnd=datenum({'21/02/18'},'dd/mm/yy');
...
%-- speed threshold -------------------------------------------
MaxSpeed=1.1; % 4 km/hour treshold above which definetly fligh
MaxSpeedCalc=0.40; % 90 percintile of walking (calculated) speeds
...
for i=4:length(Indeviduals)
    ...
    for r=1:length(dates)-1
        ...
        % if roosting in Daled start/end day just based on time
        ...
        if inDaled(1)==1
            ...
            left=find(localmldate1>=SunRiseSet2use(1) & ...
                DistNightBefore>200,1,'first');
        end
        if inDaled(2)==1
            returned=find(localmldate1>SunRiseSet2use(2),1,'first');
        else
            returned=find(DistNightAfter>200,1,'last');
        end
        ...
        if ~isempty(returned)
            ...
            %--check if returned to water but maybe slightli diffrent place
            ...
            if inRoost(end)==1 % if last point at night in in agmon, means returned
                returned=find(inRoost==0,1,'last')+1;
            else
...
save('StopInformationTable_new1','StopInformationTable_new')
